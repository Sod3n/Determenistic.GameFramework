...
namespace Deterministic.GameFramework.Box2D
{
    public static class B2Manifolds
    {
        ...
        public static B2Manifold b2CollideCapsules(in B2Capsule capsuleA, in B2Transform xfA, in B2Capsule capsuleB, in B2Transform xfB)
        {
            ...
            float distance = (float)Float.Sqrt(distanceSquared);
            ...
            if (manifold.pointCount == 0)
            {
                ...
                manifold.points[0].separation = (float)Float.Sqrt(distanceSquared) - radius;
        ...
        public static B2Manifold b2CollidePolygons(ref B2Polygon polygonA, in B2Transform xfA, ref B2Polygon polygonB, in B2Transform xfB)
        {
            ...
            if (separationA > 0.1f * linearSlop || separationB > 0.1f * linearSlop)
            {
                ...
                float distance = Float.Sqrt(result.distanceSquared);
                ...
                if (result.fraction1 == 0.0f && result.fraction2 == 0.0f)
                {
                    ...
                    B2Vec2 normal = b2Sub(v21, v11);
                    B2_ASSERT(result.distanceSquared > 0.0f);
                    float distance = (float)Float.Sqrt(result.distanceSquared);
