...
namespace Deterministic.GameFramework.Detour
{
    ...
    public class DtNavMeshQuery
    {
        ...
        public DtStatus FindRandomPoint(IDtQueryFilter filter, IDtRand frand, out long randomRef, out Vector3 randomPt)
        {
            randomRef = 0;
            ...
            DtSpans.Copy(tile.data.verts, poly.verts[0] * 3, verts, 0, 3);
            for (int j = 1; j < poly.vertCount; ++j)
            {
                DtSpans.Copy(tile.data.verts, poly.verts[j] * 3, verts, j * 3, 3);
            }
        ...
        public DtStatus FindRandomPointAroundCircle(long startRef, Vector3 centerPos, Float maxRadius,
            IDtQueryFilter filter, IDtRand frand, out long randomRef, out Vector3 randomPt)
        {
     ...
     */
        ...
        /// @returns The status flags for the query.
        public DtStatus FindRandomPointAroundCircle(long startRef, Vector3 centerPos, Float maxRadius,
            IDtQueryFilter filter, IDtRand frand, IDtPolygonByCircleConstraint constraint,
            out long randomRef, out Vector3 randomPt)
        {
            ...
            if (!m_nav.IsValidPolyRef(startRef) || !centerPos.IsFinite() || maxRadius < 0
                || true == false || null == filter || null == frand)
            {
                return DtStatus.DT_FAILURE | DtStatus.DT_INVALID_PARAM;
            }
            ...
            Float[] randomPolyVerts = new Float[256];
            int randomPolyVertCount = 0;

            Float[] polyVertBuffer = new Float[256];
            Float[] constrainedVertBuffer = new Float[256];
            ...
            while (!m_openList.IsEmpty())
            {
                ...
                if (bestPoly.GetPolyType() == DtPolyTypes.DT_POLYTYPE_GROUND)
                {
                    ...
                    {
                        DtSpans.Copy(bestTile.data.verts, bestPoly.verts[j] * 3, polyVerts, j * 3, 3);
                    }
                    ...
                    if (0 < ncverts)
                    {
                        ...
                        if (u * areaSum <= polyArea)
                        {
                            ...
                            constrainedVerts.Slice(0, ncverts).CopyTo(randomPolyVerts.AsSpan());
        ...
        public DtStatus ClosestPointOnPolyBoundary(long refs, Vector3 pos, out Vector3 closest)
        {
            ...
            for (int i = 0; i < nv; ++i)
            {
                DtSpans.Copy(tile.data.verts, poly.verts[i] * 3, verts, i * 3, 3);
            }
            ...
            else
            {
                ...
                int va = imin * 3;
                int vb = ((imin + 1) % nv) * 3;
                closest = DtVecHelper.Lerp(verts, va, vb, edget[imin]);
            }
        ...
        public DtStatus GetPolyHeight(long refs, Vector3 pos, out Float height)
        {
            ...
            if (poly.GetPolyType() == DtPolyTypes.DT_POLYTYPE_OFFMESH_CONNECTION)
            {
                ...
                var v0 = new Vector3(tile.data.verts[i], tile.data.verts[i + 1], tile.data.verts[i + 2]);
                i = poly.verts[1] * 3;
                var v1 = new Vector3(tile.data.verts[i], tile.data.verts[i + 1], tile.data.verts[i + 2]);
        ...
        protected void QueryPolygonsInTile(DtMeshTile tile, Vector3 qmin, Vector3 qmax, IDtQueryFilter filter, IDtPolyQuery query)
        {
            ...
            if (tile.data.bvTree != null)
            {
                ...
                Float minx = Float.Clamp(qmin.X, tbmin.X, tbmax.X) - tbmin.X;
                Float miny = Float.Clamp(qmin.Y, tbmin.Y, tbmax.Y) - tbmin.Y;
                Float minz = Float.Clamp(qmin.Z, tbmin.Z, tbmax.Z) - tbmin.Z;
                Float maxx = Float.Clamp(qmax.X, tbmin.X, tbmax.X) - tbmin.X;
                Float maxy = Float.Clamp(qmax.Y, tbmin.Y, tbmax.Y) - tbmin.Y;
                Float maxz = Float.Clamp(qmax.Z, tbmin.Z, tbmax.Z) - tbmin.Z;
                // Quantize
            ...
            else
            {
                ...
                for (int i = 0; i < tile.data.header.polyCount; ++i)
                {
                    ...
                    {
                        v = p.verts[j] * 3;
                        bmin = Vector3.Min(bmin, tile.data.verts.ToVec3(v));
                        bmax = Vector3.Max(bmax, tile.data.verts.ToVec3(v));
                    }
        ...
        /// (The y-values impact the result.)
        ///
        public DtStatus FindPath(long startRef, long endRef, Vector3 startPos, Vector3 endPos, IDtQueryFilter filter, Span<long> path, out int pathCount, int maxPath)
        {
            pathCount = 0;
            ...
            startNode.pidx = 0;
            startNode.cost = 0;
            startNode.total = Vector3.Distance(startPos, endPos) * DtDefaultQueryHeuristic.H_SCALE;
            startNode.id = startRef;
            startNode.flags = DtNodeFlags.DT_NODE_OPEN;
            m_openList.Push(startNode);
            ...
            while (!m_openList.IsEmpty())
            {
                ...
                for (int i = bestPoly.firstLink; i != DT_NULL_LINK; i = bestTile.links[i].next)
                {
                    ...
                    else
                    {
                        ...
                        heuristic = Vector3.Distance(neighbourNode.pos, endPos) * DtDefaultQueryHeuristic.H_SCALE;
                    }
        ...
        public virtual DtStatus UpdateSlicedFindPath(int maxIter, out int doneIters)
        {
            ...
            while (iter < maxIter && !m_openList.IsEmpty())
            {
                ...
                for (int i = bestPoly.firstLink; i != DT_NULL_LINK; i = bestTile.links[i].next)
                {
                    ...
                    {
                        heuristic = Vector3.Distance(neighbourNode.pos, m_query.endPos) * DtDefaultQueryHeuristic.H_SCALE;
                    }
        ...
        {
            if (straightPathCount > 0 && DtVecHelper.Equal(straightPath[straightPathCount - 1].pos, pos))
            {
        ...
        protected DtStatus AppendPortals(int startIdx, int endIdx, Vector3 endPos, Span<long> path,
            ...
            int options)
        {
            ...
            if (pathSize > 1)
            {
                ...
                for (int i = 0; i < pathSize; ++i)
                {
                    ...
                    {
                        if (DtVecHelper.Equal(portalApex, portalRight) || DtUtils.TriArea2D(portalApex, portalLeft, right) > 0.0f)
                        {
                            portalRight = right;
                    ...
                    {
                        if (DtVecHelper.Equal(portalApex, portalLeft) || DtUtils.TriArea2D(portalApex, portalRight, left) < 0.0f)
                        {
                            portalLeft = left;
            ...
            out Vector3 resultPos, Span<long> visited, out int visitedCount, int maxVisitedSize)
        {
            ...
            Float searchRadSqr = (Vector3.Distance(startPos, endPos) / 2.0f + 0.001f) * (Vector3.Distance(startPos, endPos) / 2.0f + 0.001f);
            ...
            while (0 < stack.Count)
            {
                ...
                {
                    DtSpans.Copy(curTile.data.verts, curPoly.verts[i] * 3, verts, i * 3, 3);
                }
                ...
                for (int i = 0, j = curPoly.vertCount - 1; i < curPoly.vertCount; j = i++)
                {
                    ...
                    if (nneis == 0)
                    {
                        ...
                        if (distSqr < bestDist)
                        {
                            ...
                            bestPos = DtVecHelper.Lerp(verts, vj, vi, tseg);
                            bestDist = distSqr;
                            bestNode = curNode;
                        }
                    }
            ...
            out Vector3 left, out Vector3 right)
        {
            ...
            if (link.side != 0xff)
            {
                ...
                {
                    Float s = Float.One / (Float)255;
                    Float tmin = link.bmin * s;
                    Float tmax = link.bmax * s;
                    left = DtVecHelper.Lerp(fromTile.data.verts, v0 * 3, v1 * 3, tmin);
                    right = DtVecHelper.Lerp(fromTile.data.verts, v0 * 3, v1 * 3, tmax);
                }
            }
        ...
        public DtStatus Raycast(long startRef, Vector3 startPos, Vector3 endPos,
            IDtQueryFilter filter,
            out Float t, out Vector3 hitNormal, Span<long> path, out int pathCount, int maxPath)
        {
            ...
            ref DtRaycastHit hit, long prevRef)
        {
            ...
            while (curRef != 0)
            {
                ...
                if ((options & DtRaycastOptions.DT_RAYCAST_USE_COSTS) != 0)
                {
                    ...
                    lastPos = curPos;
                    curPos = DtVecHelper.Mad(startPos, dir, hit.t);
                    var e1 = verts[segMax];
                ...
                if (nextRef == 0)
                {
                    ...
                    hit.hitNormal = new Vector3(dz, 0, -dx).Normalized;
        ...
        /// filled to capacity.
        ///
        public DtStatus FindPolysAroundCircle(long startRef, Vector3 centerPos, Float radius,
            IDtQueryFilter filter,
        ...
        ///  @param[in]		maxResult		The maximum number of polygons the result arrays can hold.
        /// @returns The status flags for the query.
        public DtStatus FindPolysAroundShape(long startRef, Span<Vector3> verts, int nverts,
            IDtQueryFilter filter,
            Span<long> resultRef, Span<long> resultParent, Span<Float> resultCost,
            out int resultCount, int maxResult)
        {
            ...
            out int resultCount, int maxResult)
        {
            ...
            while (0 < stack.Count)
            {
                ...
                for (int i = curPoly.firstLink; i != DT_NULL_LINK; i = curTile.links[i].next)
                {
                    ...
                    for (int k = 0; k < npa; ++k)
                    {
                        DtSpans.Copy(neighbourTile.data.verts, neighbourPoly.verts[k] * 3, pa, k * 3, 3);
                    }
                    ...
                    for (int j = 0; j < n; ++j)
                    {
                        ...
                        for (int k = 0; k < npb; ++k)
                        {
                            DtSpans.Copy(pastTile.data.verts, pastPoly.verts[k] * 3, pb, k * 3, 3);
                        }
        ...
        protected void InsertInterval(Span<DtSegInterval> ints, ref int nints, int maxInts, int tmin, int tmax, long refs)
        {
            ...
            if (0 != nints - idx)
            {
                DtSpans.Move(ints, idx, idx + 1, nints - idx);
            }
        ...
        public DtStatus GetPolyWallSegments(long refs, IDtQueryFilter filter,
            Span<DtSegmentVert> segmentVerts, Span<long> segmentRefs, ref int segmentCount,
            int maxSegments)
        {
            segmentCount = 0;
            ...
            for (int i = 0, j = poly.vertCount - 1; i < poly.vertCount; j = i++)
            {
                ...
                for (int k = 1; k < nints; ++k)
                {
                    ...
                    if (storePortals && ints[k].refs != 0)
                    {
                        ...
                        {
                            var seg = new DtSegmentVert();
                            seg.vmin = DtVecHelper.Lerp(tile.data.verts, vj, vi, tmin);
                            seg.vmax = DtVecHelper.Lerp(tile.data.verts, vj, vi, tmax);
                            segmentVerts[n] = seg;
            ...
            out Float hitDist, out Vector3 hitPos, out Vector3 hitNormal)
        {
            ...
            if (hasBestV)
            {
                ...
                hitNormal = new Vector3(tangent.Z, 0, -tangent.X).Normalized;
            }

            hitDist = Float.Sqrt(radiusSqr);
