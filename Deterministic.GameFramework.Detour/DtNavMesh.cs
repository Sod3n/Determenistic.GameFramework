...
namespace Deterministic.GameFramework.Detour
{
    ...
    public class DtNavMesh
    {
        ...
        {
            tx = (int)Float.Floor((pos.X - m_orig.X) / m_tileWidth);
            ty = (int)Float.Floor((pos.Z - m_orig.Z) / m_tileHeight);
        }
        ...
        private int QueryPolygonsInTile(DtMeshTile tile, Vector3 qmin, Vector3 qmax, Span<long> polys, int maxPolys)
        {
                    ...
                    foreach (var connectPoly in connectPolys)
                    {
                        ...
                        if (idx != DT_NULL_LINK)
                        {
                            ...
                            if (dir == 0 || dir == 4)
                            {
                                ...
                                link.bmin = (byte)(int)Float.Round(Float.Clamp(tmin, Float.FixedZero, Float.FixedOne) * 255);
                                link.bmax = (byte)(int)Float.Round(Float.Clamp(tmax, Float.FixedZero, Float.FixedOne) * 255);
                            }
                            else if (dir == 2 || dir == 6)
                            {
        ...
        void ConnectExtOffMeshLinks(DtMeshTile tile, DtMeshTile target, int side)
        {
            ...
            for (int i = 0; i < target.data.header.offMeshConCount; ++i)
            {
                ...
                var ext = new Vector3(targetCon.rad, target.data.header.walkableClimb, targetCon.rad);
        ...
        private int FindConnectingPolys(Float[] verts, int va, int vb, DtMeshTile tile, int side, ref List<DtConnectPoly> cons)
        {
            ...
            for (int i = 0; i < tile.data.header.polyCount; ++i)
            {
                ...
                for (int j = 0; j < nv; ++j)
                {
                    ...
                    Float tmin = Float.Max(amin.X, bmin.X);
                    Float tmax = Float.Min(amax.X, bmax.X);
        ...
        private bool OverlapSlabs(DtVec2f amin, DtVec2f amax, DtVec2f bmin, DtVec2f bmax, Float px, Float py)
        {
            // Check for horizontal overlap.
            // The segment is shrunken a little so that slabs which touch
            // at end points are not connected.
            Float minx = Float.Max(amin.X + px, bmin.X + px);
            Float maxx = Float.Min(amax.X - px, bmax.X - px);
            if (minx > maxx)
            {
                return false;
            }
        ...
        void BaseOffMeshLinks(DtMeshTile tile)
        {
            ...
            for (int i = 0; i < tile.data.header.offMeshConCount; ++i)
            {
                ...
                var ext = new Vector3(con.rad, tile.data.header.walkableClimb, con.rad);
                ...
                // findNearestPoly may return too optimistic results, further check
                // to make sure.
                if ((nearestPt.X - p[0].X) * (nearestPt.X - p[0].X) + (nearestPt.Z - p[0].Z) * (nearestPt.Z - p[0].Z) > con.rad * con.rad)
                {
                    continue;
                }
        ...
        Vector3 ClosestPointOnDetailEdges(DtMeshTile tile, DtPoly poly, Vector3 pos, bool onlyBoundary)
        {
                ...
                for (int i = 0; i < pd.triCount; i++)
                {
                    ...
                    for (int j = 0; j < 3; ++j)
                    {
                        if (tris[ti + j] < poly.vertCount)
                        {
                            ...
                            v[j] = new Vector3(
                                tile.data.verts[index],
                                tile.data.verts[index + 1],
                        ...
                        else
                        {
                            ...
                            v[j] = new Vector3(
                                tile.data.detailVerts[index],
                                tile.data.detailVerts[index + 1],
            ...
            {
                Span<Vector3> v = tempV.Slice(0, 2);
                for (int j = 0; j < poly.vertCount; ++j)
                {
                    int k = (j + 1) % poly.vertCount;
                    v[0] = new Vector3(
                        tile.data.verts[poly.verts[j] * 3],
                        tile.data.verts[poly.verts[j] * 3 + 1],
                        tile.data.verts[poly.verts[j] * 3 + 2]
                    );
                    v[1] = new Vector3(
                        tile.data.verts[poly.verts[k] * 3],
        ...
        public bool GetPolyHeight(DtMeshTile tile, DtPoly poly, Vector3 pos, out Float height)
        {
            ...
            for (int i = 0; i < nv; ++i)
            {
                var src = tile.data.verts.AsSpan(poly.verts[i] * 3, 3);
                var dst = verts.Slice(i * 3, 3);
                src.CopyTo(dst);
            }
            ...
            else
            {
                Span<Vector3> v = tempV;
                v[0] = new Vector3(
                    tile.data.verts[poly.verts[0] * 3],
                    tile.data.verts[poly.verts[0] * 3 + 1],
                    tile.data.verts[poly.verts[0] * 3 + 2]
                );
                for (int j = 1; j < poly.vertCount - 1; ++j)
                {
                    ...
                    {
                        v[k + 1] = new Vector3(
                            tile.data.verts[poly.verts[j + k] * 3],
                            tile.data.verts[poly.verts[j + k] * 3 + 1],
                            tile.data.verts[poly.verts[j + k] * 3 + 2]
                        );
                    }
        ...
        public void ClosestPointOnPoly(long refs, Vector3 pos, out Vector3 closest, out bool posOverPoly)
        {
            ...
            {
                closest = new Vector3(closest.X, h, closest.Z);
                posOverPoly = true;
                return;
            }
        ...
        private long FindNearestPolyInTile(DtMeshTile tile, Vector3 center, Vector3 halfExtents, out Vector3 nearestPt)
        {
            ...
            for (int i = 0; i < polyCount; ++i)
            {
                ...
                if (posOverPoly)
                {
                    d = Float.Abs(diff.Y) - tile.data.header.walkableClimb;
                    d = d > 0 ? d * d : Float.FixedZero;
                }
                else
                {
                    d = diff.SqrMagnitude;
        ...
        public Vector3 GetPolyCenter(long refs)
        {
            ...
            if (status.Succeeded())
            {
                Float cx = Float.FixedZero;
                Float cy = Float.FixedZero;
                Float cz = Float.FixedZero;
                for (int i = 0; i < poly.vertCount; ++i)
                {
                    int v = poly.verts[i] * 3;
                    cx += tile.data.verts[v];
                    cy += tile.data.verts[v + 1];
                    cz += tile.data.verts[v + 2];
                }

                Float s = Float.FixedOne / poly.vertCount;
                center = new Vector3(cx * s, cy * s, cz * s);
            }
        ...
        public void ComputeBounds(out Vector3 bmin, out Vector3 bmax)
        {
            ...
            for (int t = 0; t < GetMaxTiles(); ++t)
            {
                ...
                if (tile != null && tile.data != null)
                {
                    ...
                    {
                        bminX = Float.Min(bminX, tile.data.verts[i]);
                        bminY = Float.Min(bminY, tile.data.verts[i + 1]);
                        bminZ = Float.Min(bminZ, tile.data.verts[i + 2]);
                        bmaxX = Float.Max(bmaxX, tile.data.verts[i]);
                        bmaxY = Float.Max(bmaxY, tile.data.verts[i + 1]);
                        bmaxZ = Float.Max(bmaxZ, tile.data.verts[i + 2]);
                    }
                    bmin = new Vector3(bminX, bminY, bminZ);
                    bmax = new Vector3(bmaxX, bmaxY, bmaxZ);
                }
            }
        }
    }
