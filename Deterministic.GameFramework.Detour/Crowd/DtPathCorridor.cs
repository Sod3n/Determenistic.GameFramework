...
namespace Deterministic.GameFramework.Detour.Crowd
{
    ...
    public class DtPathCorridor
    {
        ...
        */
        ...
        {
            Float MIN_TARGET_DIST = (Float)0.01f;
            ...
            while (0 < ncorners)
            {
                if ((corners[0].flags & DtStraightPathFlags.DT_STRAIGHTPATH_OFFMESH_CONNECTION) != 0 ||
                    DtVecHelper.Dist2DSqr(corners[0].pos, m_pos) > (MIN_TARGET_DIST * MIN_TARGET_DIST))
                {
                    break;
                }
        ...
        */
        ...
        public void OptimizePathVisibility(Vector3 next, Float pathOptimizationRange, DtNavMeshQuery navquery, IDtQueryFilter filter)
        {
            ...
            Float dist = DtVecHelper.Dist2D(m_pos, next);
            ...
            dist = Float.Min(dist + (Float)0.01f, pathOptimizationRange);

            // Adjust ray length.
            var delta = next - m_pos;
            Vector3 goal = DtVecHelper.Mad(m_pos, delta, pathOptimizationRange / dist);
        ...
        */
        ...
        public void SetCorridor(Vector3 target, Span<long> path, int npath)
        {
            m_target = target;
            DtSpans.Copy<long>(path, 0, m_path, 0, npath);
            m_npath = npath;
        }
