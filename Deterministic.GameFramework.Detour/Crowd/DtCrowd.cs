...
// a source file. It reduces clutter in the main section of the header.
    ...
    */
    ...
    public class DtCrowd
    {
        ...
        private readonly Vector3 _agentPlacementHalfExtents;
        ...
        {
            _config = config;
            _agentPlacementHalfExtents = new Vector3(config.maxAgentRadius * (Float)2.0f, config.maxAgentRadius * (Float)1.5f, config.maxAgentRadius * (Float)2.0f);
            ...
            _grid = new DtProximityGrid(_config.maxAgentRadius * (Float)3);
        ...
        public DtCrowdAgent AddAgent(Vector3 pos, DtCrowdAgentParams option)
        {
            ...
            ag.topologyOptTime = (Float)0;
            ag.targetReplanTime = (Float)0;
            ag.nneis = 0;
     ...
     */
        ...
        private void UpdateMoveRequest(IList<DtCrowdAgent> agents, Float dt)
        {
            ...
            DtSortedQueue<DtCrowdAgent> queue = new DtSortedQueue<DtCrowdAgent>((a1, a2) => a2.targetReplanTime.CompareTo(a1.targetReplanTime));
            ...
            for (var i = 0; i < agents.Count; i++)
            {
                ...
                if (ag.targetState == DtMoveRequestState.DT_CROWDAGENT_TARGET_REQUESTING)
                {
                    ...
                    long[] reqPath = new long[32]; // The path to the request location
                    int reqPathCount = 0;
                    ...
                    if (ag.targetReplan) // && npath > 10)
                    {
                        ...
                        status = _navQuery.FinalizeSlicedFindPathPartial(path, npath, reqPath, out reqPathCount, MAX_RES);
                    }
                    else
                    {
                    ...
                    ag.targetReplanWaitTime = (Float)0;
                }
                    ...
                    else if (status.Succeeded())
                    {
                        ...
                        if (valid)
                        {
                            ...
                            if (npath > 1)
                            {
                                ...
                                DtSpans.Move(res, 0, npath - 1, nres);
                                // Copy old path in the beginning.
                                DtSpans.Copy(path, 0, res, 0, npath - 1);
                                nres += npath - 1;
                                ...
                                for (int j = 0; j < nres; ++j)
                                {
                                    if (j - 1 >= 0 && j + 1 < nres)
                                    {
                                        if (res[j - 1] == res[j + 1])
                                        {
                                            DtSpans.Move(res, j + 1, (j - 1), nres - (j + 1));
                                            nres -= 2;
                                            j -= 2;
                                        }
                                    }
        ...
        private void UpdateTopologyOptimization(IList<DtCrowdAgent> agents, Float dt)
        {
            ...
            DtSortedQueue<DtCrowdAgent> queue = new DtSortedQueue<DtCrowdAgent>((a1, a2) => a2.topologyOptTime.CompareTo(a1.topologyOptTime));
        ...
        private void BuildNeighbours(IList<DtCrowdAgent> agents)
        {
            ...
            for (var i = 0; i < agents.Count; i++)
            {
                ...
                Float updateThr = ag.option.collisionQueryRange * (Float)0.25f;
                if (DtVecHelper.Dist2DSqr(ag.npos, ag.boundary.GetCenter()) > (updateThr * updateThr)
        ...
        private int GetNeighbours(Vector3 pos, Float height, Float range, DtCrowdAgent skip, DtCrowdNeighbour[] result, int maxResult, DtProximityGrid grid)
        {
            int n = 0;
            ...
            for (int i = 0; i < nids; ++i)
            {
                ...
                if (Float.Abs(diff.Y) >= (height + ag.option.height) / (Float)2.0f)
                {
                    continue;
                }
        ...
        private void TriggerOffMeshConnections(IList<DtCrowdAgent> agents)
        {
            ...
            for (var i = 0; i < agents.Count; i++)
            {
                ...
                // Check
                Float triggerRadius = ag.option.radius * (Float)2.25f;
                if (ag.OverOffmeshConnection(triggerRadius))
                {
                            ...
                            ref anim.endPos, _navQuery))
                    {
                        ...
                        anim.active = true;
                        anim.t = (Float)0;
                        anim.tmax = (DtVecHelper.Dist2D(anim.startPos, anim.endPos) / ag.option.maxSpeed) * (Float)0.5f;
        ...
        private void CalculateSteering(IList<DtCrowdAgent> agents)
        {
            ...
            for (var i = 0; i < agents.Count; i++)
            {
                ...
                {
                    dvel = ag.targetPos;
                    ag.desiredSpeed = ag.targetPos.Magnitude;
                }
                else
                {
                    ...
                    Float slowDownRadius = ag.option.radius * (Float)2; // TODO: make less hacky.
                ...
                if ((ag.option.updateFlags & DtCrowdAgentUpdateFlags.DT_CROWD_SEPARATION) != 0)
                {
                    ...
                    Float invSeparationDist = Float.One / separationDist;
                    ...
                    for (int j = 0; j < ag.nneis; ++j)
                    {
                        ...
                        Float weight = separationWeight * ((Float)1 - (dist * invSeparationDist) * (dist * invSeparationDist));

                        disp = DtVecHelper.Mad(disp, diff, weight / dist);
                        w += (Float)1;
                    }
                    ...
                    if (w > (Float)0.0001f)
                    {
                        ...
                        dvel = DtVecHelper.Mad(dvel, disp, Float.One / w);
                        // Clamp desired velocity to desired speed.
                        Float speedSqr = dvel.SqrMagnitude;
            ...
            for (var i = 0; i < agents.Count; i++)
            {
                ...
                if ((ag.option.updateFlags & DtCrowdAgentUpdateFlags.DT_CROWD_OBSTACLE_AVOIDANCE) != 0)
                {
                    ...
                    for (int j = 0; j < ag.boundary.GetSegmentCount(); ++j)
                    {
                        ...
                        Vector3 s3 = s[1];
                        if (DtUtils.TriArea2D(ag.npos, s[0], s3) < (Float)0)
                        {
                            continue;
                        }
            ...
            for (int iter = 0; iter < 4; ++iter)
            {
                for (var i = 0; i < agents.Count; i++)
                {
                    ...
                    for (int j = 0; j < ag.nneis; ++j)
                    {
                        ...
                        if (dist > (ag.option.radius + nei.option.radius) * (ag.option.radius + nei.option.radius))
                        {
                            continue;
                        }
                        ...
                        if (dist < (Float)0.0001f)
                        {
                            // Agents on top of each other, try to choose diverging separation directions.
                            if (idx0 > idx1)
                            {
                                diff = new Vector3(-ag.dvel.Z, (Float)0, ag.dvel.X);
                            }
                            else
                            {
                                diff = new Vector3(ag.dvel.Z, (Float)0, -ag.dvel.X);
                        ...
                        {
                            pen = (Float.One / dist) * (pen * (Float)0.5f) * _config.collisionResolveFactor;
                        }

                        ag.disp = DtVecHelper.Mad(ag.disp, diff, pen);
        ...
        private void UpdateOffMeshConnections(IList<DtCrowdAgent> agents, Float dt)
        {
            ...
            for (var i = 0; i < agents.Count; i++)
            {
                ...
                // Update position
                Float ta = anim.tmax * (Float)0.15f;
                Float tb = anim.tmax;
                if (anim.t < ta)
                {
                    Float u = Tween(anim.t, (Float)0, ta);
        ...
        {
            return Float.Clamp((t - t0) / (t1 - t0), (Float)0, (Float)1);
        }
    }
}
