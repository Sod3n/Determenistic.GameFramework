/*
Copyright (c) 2009-2010 Mikko Mononen memon@inside.org
recast4j copyright (c) 2015-2019 Piotr Piastucki piotr@jtilia.org
DotRecast Copyright (c) 2023-2024 Choi Ikpil ikpil@naver.com

This software is provided 'as-is', without any express or implied
warranty.  In no event will the authors be held liable for any damages
arising from the use of this software.
Permission is granted to anyone to use this software for any purpose,
including commercial applications, and to alter it and redistribute it
freely, subject to the following restrictions:
1. The origin of this software must not be misrepresented; you must not
 claim that you wrote the original software. If you use this software
 in a product, an acknowledgment in the product documentation would be
 appreciated but is not required.
2. Altered source versions must be plainly marked as such, and must not be
 misrepresented as being the original software.
3. This notice may not be removed or altered from any source distribution.
*/

using System;
using Deterministic.GameFramework.Types;
using Deterministic.GameFramework.Detour;

namespace Deterministic.GameFramework.Detour.Crowd
{
    public class DtObstacleAvoidanceQuery
    {
        public const int DT_MAX_PATTERN_DIVS = 32; // < Max numver of adaptive divs.
        public const int DT_MAX_PATTERN_RINGS = 4;
        public static readonly Float DT_PI = (Float)3.14159265f;

        private DtObstacleAvoidanceParams m_params;
        private Float m_invHorizTime;
        private Float m_vmax;
        private Float m_invVmax;

        private readonly int m_maxCircles;
        private readonly DtObstacleCircle[] m_circles;
        private int m_ncircles;

        private readonly int m_maxSegments;
        private readonly DtObstacleSegment[] m_segments;
        private int m_nsegments;

        public DtObstacleAvoidanceQuery(int maxCircles, int maxSegments)
        {
            m_maxCircles = maxCircles;
            m_ncircles = 0;
            m_circles = new DtObstacleCircle[m_maxCircles];
            for (int i = 0; i < m_maxCircles; i++)
            {
                m_circles[i] = new DtObstacleCircle();
            }

            m_maxSegments = maxSegments;
            m_nsegments = 0;
            m_segments = new DtObstacleSegment[m_maxSegments];
            for (int i = 0; i < m_maxSegments; i++)
            {
                m_segments[i] = new DtObstacleSegment();
            }
        }

        public void Reset()
        {
            m_ncircles = 0;
            m_nsegments = 0;
        }

        public void AddCircle(Vector3 pos, Float rad, Vector3 vel, Vector3 dvel)
        {
            if (m_ncircles >= m_maxCircles)
                return;

            DtObstacleCircle cir = m_circles[m_ncircles++];
            cir.p = pos;
            cir.rad = rad;
            cir.vel = vel;
            cir.dvel = dvel;
        }

        public void AddSegment(Vector3 p, Vector3 q)
        {
            if (m_nsegments >= m_maxSegments)
                return;

            DtObstacleSegment seg = m_segments[m_nsegments++];
            seg.p = p;
            seg.q = q;
        }

        public int GetObstacleCircleCount()
        {
            return m_ncircles;
        }

        public DtObstacleCircle GetObstacleCircle(int i)
        {
            return m_circles[i];
        }

        public int GetObstacleSegmentCount()
        {
            return m_nsegments;
        }

        public DtObstacleSegment GetObstacleSegment(int i)
        {
            return m_segments[i];
        }

        private void Prepare(Vector3 pos, Vector3 dvel)
        {
            // Prepare obstacles
            for (int i = 0; i < m_ncircles; ++i)
            {
                DtObstacleCircle cir = m_circles[i];

                // Side
                Vector3 pa = pos;
                Vector3 pb = cir.p;

                Vector3 orig = new Vector3();
                Vector3 dv = new Vector3();
                cir.dp = pb - pa;
                cir.dp = cir.dp.Normalized;
                dv = cir.dvel - dvel;

                Float a = DtUtils.TriArea2D(orig, cir.dp, dv);
                if (a < (Float)0.01f)
                {
                    cir.np.X = -cir.dp.Z;
                    cir.np.Z = cir.dp.X;
                }
                else
                {
                    cir.np.X = cir.dp.Z;
                    cir.np.Z = -cir.dp.X;
                }
            }

            for (int i = 0; i < m_nsegments; ++i)
            {
                DtObstacleSegment seg = m_segments[i];

                // Precalc if the agent is really close to the segment.
                Float r = (Float)0.01f;
                var distSqr = DtUtils.DistancePtSegSqr2D(pos, seg.p, seg.q, out var t);
                seg.touch = distSqr < (r * r);
            }
        }

        private bool SweepCircleCircle(Vector3 c0, Float r0, Vector3 v, Vector3 c1, Float r1, out Float tmin, out Float tmax)
        {
            Float EPS = (Float)0.0001f;

            tmin = (Float)0;
            tmax = (Float)0;

            Vector3 s = c1 - c0;
            Float r = r0 + r1;
            Float c = s.Dot2D(s) - r * r;
            Float a = v.Dot2D(v);
            if (a < EPS)
                return false; // not moving

            // Overlap, calc time to exit.
            Float b = v.Dot2D(s);
            Float d = b * b - a * c;
            if (d < (Float)0)
                return false; // no intersection.

            a = Float.One / a;
            Float rd = Float.Sqrt(d);

            tmin = (b - rd) * a;
            tmax = (b + rd) * a;

            return true;
        }

        private bool IsectRaySeg(Vector3 ap, Vector3 u, Vector3 bp, Vector3 bq, ref Float t)
        {
            Vector3 v = bq - bp;
            Vector3 w = ap - bp;
            Float d = DtVecHelper.Perp2D(u, v);
            if (Float.Abs(d) < (Float)1e-6f)
                return false;

            d = Float.One / d;
            t = DtVecHelper.Perp2D(v, w) * d;
            if (t < (Float)0 || t > (Float)1)
                return false;

            Float s = DtVecHelper.Perp2D(u, w) * d;
            if (s < (Float)0 || s > (Float)1)
                return false;

            return true;
        }

        /**
     * Calculate the collision penalty for a given velocity vector
     *
     * @param vcand
     *            sampled velocity
     * @param dvel
     *            desired velocity
     * @param minPenalty
     *            threshold penalty for early out
     */
        private Float ProcessSample(Vector3 vcand, Float cs, Vector3 pos, Float rad, Vector3 vel, Vector3 dvel,
            Float minPenalty, DtObstacleAvoidanceDebugData debug)
        {
            // penalty for straying away from the desired and current velocities
            Float vpen = m_params.weightDesVel * (DtVecHelper.Dist2D(vcand, dvel) * m_invVmax);
            Float vcpen = m_params.weightCurVel * (DtVecHelper.Dist2D(vcand, vel) * m_invVmax);

            // find the threshold hit time to bail out based on the early out penalty
            // (see how the penalty is calculated below to understand)
            // Early-out: if penalties already exceed current best, skip obstacle checks.
            Float minPen = minPenalty - vpen - vcpen;
            if (minPen < (Float)0)
                return minPenalty;
            Float tThresold = (m_params.weightToi / minPen - (Float)0.1f) * m_params.horizTime;
            if (tThresold > m_params.horizTime)
                return minPenalty;

            // Find min time of impact and exit amongst all obstacles.
            Float tmin = m_params.horizTime;
            Float side = (Float)0;
            int nside = 0;

            for (int i = 0; i < m_ncircles; ++i)
            {
                DtObstacleCircle cir = m_circles[i];

                // RVO
                Vector3 vab = vcand * (Float)2;
                vab = vab - vel;
                vab = vab - cir.vel;

                // Side
                side += Float.Clamp(Float.Min(cir.dp.Dot2D(vab) * (Float)0.5f + (Float)0.5f, cir.np.Dot2D(vab) * (Float)2), (Float)0, (Float)1);
                nside++;

                if (!SweepCircleCircle(pos, rad, vab, cir.p, cir.rad, out var htmin, out var htmax))
                    continue;

                // Handle overlapping obstacles.
                if (htmin < (Float)0 && htmax > (Float)0)
                {
                    // Avoid more when overlapped.
                    htmin = -htmin * (Float)0.5f;
                }

                if (htmin >= (Float)0 && htmin < tmin)
                {
                    tmin = htmin;
                    if (tmin < tThresold)
                        return minPenalty;
                }
            }

            for (int i = 0; i < m_nsegments; ++i)
            {
                DtObstacleSegment seg = m_segments[i];
                Float htmin = (Float)0;

                if (seg.touch)
                {
                    Vector3 sdir = seg.q - seg.p;
                    Vector3 snorm = new Vector3();
                    snorm.X = -sdir.Z;
                    snorm.Z = sdir.X;
                    if (snorm.Dot2D(vcand) < (Float)0)
                        continue;
                    htmin = (Float)0;
                }
                else
                {
                    if (!IsectRaySeg(pos, vcand, seg.p, seg.q, ref htmin))
                        continue;
                }

                htmin *= (Float)2.0f;

                if (htmin < tmin)
                {
                    tmin = htmin;
                    if (tmin < tThresold)
                        return minPenalty;
                }
            }

            // Normalize side bias, to prevent it dominating too much.
            if (nside != 0)
                side /= (Float)nside;

            Float spen = m_params.weightSide * side;
            Float tpen = m_params.weightToi * (Float.One / ((Float)0.1f + tmin * m_invHorizTime));

            Float penalty = vpen + vcpen + spen + tpen;

            // Store different penalties for debug viewing
            if (debug != null)
                debug.AddSample(vcand, cs, penalty, vpen, vcpen, spen, tpen);

            return penalty;
        }

        public int SampleVelocityGrid(Vector3 pos, Float rad, Float vmax, Vector3 vel, Vector3 dvel, out Vector3 nvel,
            DtObstacleAvoidanceParams option, DtObstacleAvoidanceDebugData debug)
        {
            Prepare(pos, dvel);
            m_params = option;
            m_invHorizTime = Float.One / m_params.horizTime;
            m_vmax = vmax;
            m_invVmax = vmax > (Float)0 ? Float.One / vmax : Float.MaxValue;

            nvel = Vector3.Zero;

            if (debug != null)
                debug.Reset();

            Float cvx = dvel.X * m_params.velBias;
            Float cvz = dvel.Z * m_params.velBias;
            Float cs = vmax * (Float)2 * ((Float)1 - m_params.velBias) / (Float)(m_params.gridSize - 1);
            Float half = (Float)(m_params.gridSize - 1) * cs * (Float)0.5f;

            Float minPenalty = Float.MaxValue;
            int ns = 0;

            for (int y = 0; y < m_params.gridSize; ++y)
            {
                for (int x = 0; x < m_params.gridSize; ++x)
                {
                    Vector3 vcand = new Vector3(cvx + (Float)x * cs - half, (Float)0, cvz + (Float)y * cs - half);
                    if ((vcand.X * vcand.X) + (vcand.Z * vcand.Z) > (vmax + cs / (Float)2) * (vmax + cs / (Float)2))
                        continue;

                    Float penalty = ProcessSample(vcand, cs, pos, rad, vel, dvel, minPenalty, debug);
                    ns++;
                    if (penalty < minPenalty)
                    {
                        minPenalty = penalty;
                        nvel = vcand;
                    }
                }
            }

            return ns;
        }

        // vector normalization that ignores the y-component.
        void DtNormalize2D(Float[] v)
        {
            Float d = Float.Sqrt(v[0] * v[0] + v[2] * v[2]);
            if (d == (Float)0)
                return;
            d = Float.One / d;
            v[0] *= d;
            v[2] *= d;
        }

        // vector normalization that ignores the y-component.
        Vector3 DtRotate2D(Float[] v, Float ang)
        {
            Vector3 dest = new Vector3();
            Float c = Float.Cos(ang);
            Float s = Float.Sin(ang);
            dest.X = v[0] * c - v[2] * s;
            dest.Z = v[0] * s + v[2] * c;
            dest.Y = v[1];
            return dest;
        }


        public int SampleVelocityAdaptive(Vector3 pos, Float rad, Float vmax, Vector3 vel, Vector3 dvel, out Vector3 nvel,
            DtObstacleAvoidanceParams option,
            DtObstacleAvoidanceDebugData debug)
        {
            Prepare(pos, dvel);
            m_params = option;
            m_invHorizTime = Float.One / m_params.horizTime;
            m_vmax = vmax;
            m_invVmax = vmax > (Float)0 ? Float.One / vmax : Float.MaxValue;

            nvel = Vector3.Zero;

            if (debug != null)
                debug.Reset();

            // Build sampling pattern aligned to desired velocity.
            // Heap-allocate instead of stackalloc: Float is a struct wrapping Fixed64
            // and stackalloc may not zero-initialize correctly on all runtimes.
            var pat = new Float[(DT_MAX_PATTERN_DIVS * DT_MAX_PATTERN_RINGS + 1) * 2];
            int npat = 0;

            int ndivs = m_params.adaptiveDivs;
            int nrings = m_params.adaptiveRings;
            int depth = m_params.adaptiveDepth;

            int nd = Math.Clamp(ndivs, 1, DT_MAX_PATTERN_DIVS);
            int nr = Math.Clamp(nrings, 1, DT_MAX_PATTERN_RINGS);
            Float da = (Float.One / (Float)nd) * DT_PI * (Float)2;
            Float ca = Float.Cos(da);
            Float sa = Float.Sin(da);

            // desired direction
            var ddir = new Float[6];
            ddir[0] = dvel.X;
            ddir[1] = dvel.Y;
            ddir[2] = dvel.Z;
            DtNormalize2D(ddir);
            Vector3 rotated = DtRotate2D(ddir, da * (Float)0.5f); // rotated by da/2
            ddir[3] = rotated.X;
            ddir[4] = rotated.Y;
            ddir[5] = rotated.Z;


            // Always add sample at zero
            pat[npat * 2 + 0] = (Float)0;
            pat[npat * 2 + 1] = (Float)0;
            npat++;

            for (int j = 0; j < nr; ++j)
            {
                Float r = (Float)(nr - j) / (Float)nr;
                pat[npat * 2 + 0] = ddir[(j % 2) * 3] * r;
                pat[npat * 2 + 1] = ddir[(j % 2) * 3 + 2] * r;
                int last1 = npat * 2;
                int last2 = last1;
                npat++;

                for (int i = 1; i < nd - 1; i += 2)
                {
                    // get next point on the "right" (rotate CW)
                    pat[npat * 2 + 0] = pat[last1] * ca + pat[last1 + 1] * sa;
                    pat[npat * 2 + 1] = -pat[last1] * sa + pat[last1 + 1] * ca;
                    // get next point on the "left" (rotate CCW)
                    pat[npat * 2 + 2] = pat[last2] * ca - pat[last2 + 1] * sa;
                    pat[npat * 2 + 3] = pat[last2] * sa + pat[last2 + 1] * ca;

                    last1 = npat * 2;
                    last2 = last1 + 2;
                    npat += 2;
                }

                if ((nd & 1) == 0)
                {
                    pat[npat * 2 + 2] = pat[last2] * ca - pat[last2 + 1] * sa;
                    pat[npat * 2 + 3] = pat[last2] * sa + pat[last2 + 1] * ca;
                    npat++;
                }
            }

            // Start sampling.
            Float cr = vmax * ((Float)1 - m_params.velBias);
            Vector3 res = new Vector3(dvel.X * m_params.velBias, (Float)0, dvel.Z * m_params.velBias);
            int ns = 0;
            for (int k = 0; k < depth; ++k)
            {
                Float minPenalty = Float.MaxValue;
                Vector3 bvel = Vector3.Zero;

                for (int i = 0; i < npat; ++i)
                {
                    Vector3 vcand = new Vector3(res.X + pat[i * 2 + 0] * cr, (Float)0, res.Z + pat[i * 2 + 1] * cr);
                    if ((vcand.X * vcand.X) + (vcand.Z * vcand.Z) > (vmax + (Float)0.001f) * (vmax + (Float)0.001f))
                        continue;

                    Float penalty = ProcessSample(vcand, cr / (Float)10, pos, rad, vel, dvel, minPenalty, debug);
                    ns++;
                    if (penalty < minPenalty)
                    {
                        minPenalty = penalty;
                        bvel = vcand;
                    }
                }

                res = bvel;

                cr *= (Float)0.5f;
            }

            nvel = res;

            return ns;
        }
    }
}
