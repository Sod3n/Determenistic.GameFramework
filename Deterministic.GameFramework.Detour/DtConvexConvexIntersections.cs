...
namespace Deterministic.GameFramework.Detour
{
    ...
    {
        private static readonly Float EPSILON = (Float)0.0001f;
        ...
        public static bool Intersect(ReadOnlySpan<Float> p, ReadOnlySpan<Float> q, Span<Float> intersection, out int nin)
        {
            ...
            do
            {
                ...
                if (Float.Abs(cross) < EPSILON)
                {
                    cross = (Float)0f;
                }

                bool parallel = cross == (Float)0f;
                ...
                if (code == DtConvexConvexIntersection.Overlap && A.Dot2D(B) < (Float)0)
                {
                    ii = AddVertex(inters, ii, ip);
                ...
                if (parallel && aHB < (Float)0f && bHA < (Float)0f)
                {
                    return false;
                }
                /* Special case: A & B collinear. */
                else if (parallel && Float.Abs(aHB) < EPSILON && Float.Abs(bHA) < EPSILON)
                {
        ...
        private static DtConvexConvexIntersection SegSegInt(Vector3 a, Vector3 b, Vector3 c, Vector3 d, ref Vector3 p, ref Vector3 q)
        {
            ...
            {
                if (s >= (Float)0.0f && s <= (Float)1.0f && t >= (Float)0.0f && t <= (Float)1.0f)
                {
                    p.X = a.X + (b.X - a.X) * s;
                    p.Y = a.Y + (b.Y - a.Y) * s;
                    p.Z = a.Z + (b.Z - a.Z) * s;
        ...
        {
            if (Float.Abs(a.X - b.X) > Float.Abs(a.Z - b.Z))
            {
