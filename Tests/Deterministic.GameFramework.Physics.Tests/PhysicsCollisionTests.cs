...
{
    private static (EntityWorld state, GameLoop gameLoop) CreateWorldWithPhysics()
    {
        ServiceLocator.Reset();
        // Register assemblies first!
        ServiceLocator.RegisterAssembly(typeof(EntityWorld).Assembly); // ECS
        ServiceLocator.RegisterAssembly(typeof(Transform2D).Assembly); // TwoD (Base)
